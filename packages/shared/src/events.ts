...
export interface ServerToClientEvents {
  ...
  /** Game has ended */
  "game:ended": (data: { winnerId: string; reason: string }) => void;

  /** Dice roll result */
  "turn:rolled": (data: { playerId: string; dice: [number, number]; newPosition: number; passedGo: boolean }) => void;

  /** Player landed on a buyable property */
  "turn:buy_option": (data: { tileIndex: number; price: number }) => void;

  /** Player bought a property */
  "turn:bought": (data: { playerId: string; tileIndex: number }) => void;

  /** Player paid rent */
  "turn:rent_paid": (data: { payerId: string; ownerId: string; amount: number; tileIndex: number }) => void;

  /** Player paid tax */
  "turn:tax_paid": (data: { playerId: string; amount: number }) => void;

  /** Player went bankrupt */
  "turn:bankrupt": (data: { playerId: string }) => void;

  /** Turn advanced to next player */
  "turn:next": (data: { currentPlayerIndex: number; turn: number }) => void;

  /** Player sent to jail */
  "turn:jail": (data: { playerId: string }) => void;

  /** Card drawn (Chance or Community Chest) */
  "turn:card": (data: { playerId: string; cardText: string }) => void;
